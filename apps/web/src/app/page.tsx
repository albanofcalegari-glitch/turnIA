  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-white to-gray-50 dark:from-gray-900 dark:to-gray-950">
      ...
      <section className="max-w-4xl mx-auto px-4 py-16 text-center sm:px-6 sm:py-28">
        <div className="inline-flex items-center gap-1.5 bg-brand-50 text-brand-700 text-xs font-semibold tracking-wide uppercase px-4 py-1.5 rounded-full mb-5 border border-brand-100 dark:bg-brand-950 dark:text-brand-400 dark:border-brand-800 sm:text-sm sm:mb-7">
          <span className="h-1.5 w-1.5 rounded-full bg-brand-500 animate-pulse" />
          45 días de prueba · Sin tarjeta
        </div>
        <h1 className="text-4xl font-extrabold text-gray-900 dark:text-white mb-5 tracking-tight sm:text-6xl sm:mb-7">
          Gestión de turnos{' '}
          <span className="bg-gradient-to-r from-brand-600 to-brand-400 bg-clip-text text-transparent">simple y profesional</span>
        </h1>
        <p className="text-base text-gray-500 dark:text-gray-400 mb-10 max-w-2xl mx-auto leading-relaxed sm:text-xl sm:mb-12">
        ...
        <p className="text-xs text-gray-400 dark:text-gray-500 mt-5 sm:text-sm">
...
function FeatureCard({ icon, title, description }: { icon: React.ReactNode; title: string; description: string }) {
  return (
    <div className="rounded-xl border border-gray-200/80 bg-white p-5 shadow-card transition-shadow duration-200 hover:shadow-card-hover dark:border-gray-700 dark:bg-gray-800">
